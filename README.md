...
| `VK_DIRECT_AUTH_CLIENT_ID` | Опциональна | ID приложения для VK OAuth-флоу входа. По умолчанию — публичные креды официального VK Android-клиента. |
| `VK_DIRECT_AUTH_CLIENT_SECRET` | Опциональна | Секрет приложения для того же флоу. По умолчанию — публичная константа официального клиента. |
...
Фича работает из коробки: своё VK-приложение не требуется. По умолчанию бот использует публичные креды официального VK Android-клиента (client_id `2274003`) — общеизвестные константы, вшитые в официальный APK и используемые в тысячах опенсорс-проектов; это не пользовательский секрет и хранится прямо в коде.

| Переменная | Обязательность | Назначение |
|---|---|---|
| `VK_DIRECT_AUTH_CLIENT_ID` | Опциональна | ID своего standalone-приложения VK. По умолчанию — публичный client_id официального VK Android-клиента (`2274003`). Своё приложение имеет смысл задавать, только если публичные дефолты для вас заблокированы VK. |
| `VK_DIRECT_AUTH_CLIENT_SECRET` | Опциональна | Защищённый ключ того же приложения. По умолчанию — публичная константа официального клиента. Реальное значение (если задано своё) держите только в `.env` или переменных окружения. |
| `VK_LOGIN_ENABLED` | Опциональна | По умолчанию `1` (флоу включён, креды есть всегда). Явное значение `0` отключает кнопку VK-входа. |
...
На bothost.ru `VK_DIRECT_AUTH_*` задаются через панель «Переменные окружения» только при желании переопределить публичные дефолты, после чего нажимается «Обновить из Git».
...
Совет: входите данными своего VK-аккаунта и не делитесь ими с третьими лицами — бот хранит их только шифрованно и не логирует.
...
| Блокировка приложения / `too_much_tries` от VK | Публичные креды официального клиента могут быть временно ограничены VK; подождите или задайте свои `VK_DIRECT_AUTH_*`. |

Вход по логину/паролю поддерживает 2FA и капчу: при запросе кода подтверждения бот просит прислать его следующим сообщением, при капче — текст с картинки.

Предупреждение: direct password grant — легаси-метод авторизации. VK может в любой момент ограничить его или публичные креды официального клиента; тогда задайте своё приложение через `VK_DIRECT_AUTH_CLIENT_ID` / `VK_DIRECT_AUTH_CLIENT_SECRET` или используйте ручной ввод токена.
